...
def user(user):
    ...
    cursor = g.conn.execute(
        "SELECT O.acc_id, A.acc_name, O.since FROM Open O, Account A WHERE user_id = %s AND A.acc_id = O.acc_id", user)
    acc_id = []
    since = []
    acc_name = []
    ...
    context = dict(user_id=user_id, user_name=user_name, mobile=mobile, email=email, address=address,
                   passport_no=passport_no, acc_id=acc_id, acc_name=acc_name, since=since)
...
def accountcheck():
    ...
    cursor = g.conn.execute(
        'SELECT * FROM Account WHERE Acc_id = %s AND Acc_id in (SELECT Acc_id FROM Open WHERE user_id = %s)', acc_id,
        user_id)
    acc = []
    acc_name = []
    cash_balance = []
    ...
    cursor = g.conn.execute('SELECT * FROM Owns O, Investment_Product IP WHERE O.Acc_id = %s AND O.IP_id = IP.IP_id',
                            acc_id)
    O_IP_id = []
    O_amount = []
    O_ip_name = []
    ...
    cursor = g.conn.execute(
        'SELECT * FROM Contains C, Investment_Product IP WHERE C.Acc_id = %s AND C.IP_id = IP.IP_id', acc_id)
    C_IP_id = []
    C_list_id = []
    C_add_time = []
    C_ip_name = []
    ...
    if len(acc) == 0:
        ...
        # get all accounts belong to the user
        cursor = g.conn.execute(
            "SELECT O.acc_id, A.acc_name, O.since FROM Open O, Account A WHERE user_id = %s AND A.acc_id = O.acc_id",
            user_id)
        acc_id = []
        since = []
        acc_name = []
        ...
        context = dict(acc=acc, acc_name=acc_name, cash_balance=cash_balance, inv_balance=inv_balance,
                       total_value=total_value, pay_id=pay_id, type=type, card_no=card_no, card_name=card_name,
                       card_expire=card_expire, create_date=create_date, O_IP_id=O_IP_id, O_amount=O_amount,
                       O_ip_name=O_ip_name, O_risk_type=O_risk_type, O_curr_yield=O_curr_yield, C_IP_id=C_IP_id,
                       C_list_id=C_list_id, C_add_time=C_add_time, C_ip_name=C_ip_name, C_risk_type=C_risk_type,
                       C_curr_yield=C_curr_yield)
...
def addpaymentmethod():
    ...
    if pay_id != '' and type != '' and card_no != '' and card_name != '' and card_expire != '':
        g.conn.execute('INSERT INTO Has_Payment_method VALUES (%s,%s,%s,%s,%s,%s,%s)', pay_id, type, card_no, card_name,
        ...
        if type == '':
            ...
            g.conn.execute('UPDATE Has_Payment_method SET type = %s WHERE (pay_id,acc_id) = (%s,%s)', type, pay_id,
                           acc_id)
        if card_no == '':
            g.conn.execute('UPDATE Has_Payment_method SET card_no = NULL WHERE (pay_id,acc_id) = (%s,%s)', pay_id,
                           acc_id)
        else:
            g.conn.execute('UPDATE Has_Payment_method SET card_no = %s WHERE (pay_id,acc_id) = (%s,%s)', card_no,
                           pay_id, acc_id)
        if card_name == '':
            g.conn.execute('UPDATE Has_Payment_method SET card_name = NULL WHERE (pay_id,acc_id) = (%s,%s)', pay_id,
                           acc_id)
        else:
            g.conn.execute('UPDATE Has_Payment_method SET card_name = %s WHERE (pay_id,acc_id) = (%s,%s)', card_name,
                           pay_id, acc_id)
        if card_expire == '':
            g.conn.execute('UPDATE Has_Payment_method SET card_expire = NULL WHERE (pay_id,acc_id) = (%s,%s)', pay_id,
                           acc_id)
        else:
            g.conn.execute('UPDATE Has_Payment_method SET card_expire = %s WHERE (pay_id,acc_id) = (%s,%s)',
                           card_expire, pay_id, acc_id)
...
def ip_query():
    ...
    if curr_yield == 'default' and min_inv_value == 'default' and freeze_time == 'default' and create_date == 'default' and expire_date == 'default':
        ...
        order = " ORDER BY "
        if curr_yield == 'high':
            order += "ip.curr_yield DESC,"
        elif curr_yield == 'low':
            order += "ip.curr_yield ASC,"
        if min_inv_value == 'high':
            order += "ip.min_inv_value DESC,"
        elif min_inv_value == 'low':
            order += "ip.min_inv_value ASC,"
        if freeze_time == 'long':
            order += "ip.freezing_time DESC,"
        elif freeze_time == 'short':
            order += "ip.freezing_time ASC,"
        if create_date == 'latest':
            order += "ip.create_date DESC,"
        elif create_date == 'early':
            order += "ip.create_date ASC,"
        if expire_date == 'latest':
            order += "ip.expire_date DESC,"
        elif expire_date == 'early':
            order += "ip.expire_date ASC,"
...
def ip_action():
    ...
    if action == 'Buy':
        ...
        c2_check = g.conn.execute("SELECT * FROM contains WHERE list_id=%s and acc_id=%s and ip_id=%s", list_id, acc_id,
                                  ip_id)
        ...
        # get all the account information
        cursor = g.conn.execute(
            'SELECT * FROM Account WHERE Acc_id = %s AND Acc_id in (SELECT Acc_id FROM Open WHERE user_id = %s)',
            acc_id, user_id)
        acc = []
        acc_name = []
        cash_balance = []
        ...
        # get owning investment product information
        cursor = g.conn.execute(
            'SELECT * FROM Owns O, Investment_Product IP WHERE O.Acc_id = %s AND O.IP_id = IP.IP_id', acc_id)
        O_IP_id = []
        O_amount = []
        O_ip_name = []
        O_risk_type = []
...
def removewatching():
    ...
    c2_check = g.conn.execute("DELETE FROM contains WHERE list_id=%s and acc_id=%s and ip_id=%s", list_id, acc_id,
                              ip_id)
    c2_check.close()
...
def buy_check():
    if request.method == 'POST':
        ...
        if float(cash_bal) < float(amount):
            c2 = g.conn.execute("SELECT pay_id FROM has_payment_method WHERE pay_id=%s and acc_id = %s", payment,
                                acc_id)
            data = list(c2)
            if len(data) == 0:
                transaction = False
        ...
        if transaction == False:
          ...
          ''', acc_id)
            ...
            c_acc_update = g.conn.execute(
                "UPDATE account SET cash_balance=%s, inv_balance=%s, total_value=%s WHERE acc_id=%s", cash_bal_new,
                inv_bal_new, tot_bal_new, acc_id)
            c_acc_update.close()
            c_acc.close()
            ...
            c2 = g.conn.execute(
                "SELECT t.acc_id, t.ip_id, t.bill_id, b.date, b.type, b.amount, b.note FROM bill b, trade t WHERE b.bill_id=t.bill_id and t.acc_id=%s",
                acc_id)
            bill = []
            for result in c2:
    ...
    if request.method == 'POST':
        ...
        if transaction == False:
          ...
          ''', acc_id)
            ...
            inv_bal_new = float(inv_bal_ori) - float(amount)
            cash_bal_new = float(cash_bal_ori) + float(amount)
            c_acc_update = g.conn.execute("UPDATE account SET cash_balance=%s, inv_balance=%s WHERE acc_id=%s",
                                          cash_bal_new, inv_bal_new, acc_id)
            c_acc_update.close()
            c_acc.close()
